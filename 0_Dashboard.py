...
# --- CONFIGURARE CONEXIUNE SIGURĂ LA BAZA DE DATE (READ-ONLY) ---
BASE_DIR = Path(__file__).parent.parent
DB_PATH = BASE_DIR / "jobs.db"

@st.cache_resource
def get_connection():
    # mode=ro deschide baza de date strict pentru CITIRE.
    # Elimină complet eroarea WinError 32 când update_jobs rulează în paralel.
...
def get_total_jobs():
    ...
    try:
        ...
        with closing(conn.cursor()) as cursor:
            # Verificăm dacă există tabelul 'jobs'
...
if search_query:
    ...
    # Preia numărul de joburi folosind funcția sigură
    numar_joburi_reale = get_total_jobs()

    stats = [
        ("💼", str(len(JOBS)), "Ocupații listate"),
        ("🎯", str(total_hard_skills), "Hard skills catalogate"),
        ("📌", str(numar_joburi_reale), "Job-uri active azi"),
    ]
